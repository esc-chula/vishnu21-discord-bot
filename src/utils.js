...
module.exports = class Util {
    ...
    static async pagination(msg, author, contents, init = true, currPage = 0) {
        if (init)
            for (const emoji of this.paginationEmojis) await msg.react(emoji);
        ...
        collector
            .on("collect", (reaction) => {
                ...
                currPage =
                    ((currPage % contents.length) + contents.length) %
                    contents.length;
    ...
    static missingPerms(memberPerms, requiredPerms) {
        return new Permissions(memberPerms).missing(
            new Permissions(requiredPerms)
        );
    }
